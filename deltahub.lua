...
local Window = ReGui:TabsWindow({
	Title = "[☠️] DeltaHub - ".. name .. " " .. scriptversion,
...
if isfile and readfile and listfiles and writefile and makefolder then
    if not isfolder("DeltaHub\\Builds") then
        makefolder("DeltaHub\\Builds")
    end
else
...
local function readbuild(name)
    local fixedname = name:gsub("DeltaHub\\Builds","")
    fixedname = fixedname:gsub("DeltaHub/Builds","")
    fixedname = fixedname:gsub("/","")
    fixedname = fixedname:gsub("DeltaHubBuilds", "")
    fixedname = fixedname:gsub(".json","")

    local path = "DeltaHub\\Builds\\"..fixedname..".json"
...
local function save(name,table)
    local path = "DeltaHub\\Builds\\"..name..".json"
...
local function savecfg()
    ...
    local fixedname = name:gsub("DeltaHub","")
    fixedname = fixedname:gsub("DeltaHub/Builds","")
    fixedname = fixedname:gsub("/","")
    fixedname = fixedname:gsub("\\","")
    fixedname = fixedname:gsub("DeltaHub/", "")
    fixedname = fixedname:gsub(".json","")
...
if not isfolder("DeltaHub/Builds") then
    writefile("DeltaHub\\Builds\\test.json","[]")
end
if not isfolder("DeltaHub/Builds") then
    makefolder("DeltaHub/Builds/test.json","[]")
end
...
bs1:Combo({
	...
	GetItems = function()
        local items = {}
        if isfolder("DeltaHub\\Builds") then
            for ___, item in ipairs(listfiles("DeltaHub\\Builds")) do
                local fixedname = item:gsub("DeltaHub\\Builds\\","")
                fixedname = fixedname:gsub("DeltaHub/Builds","")
...
row1:Button({
	...
	Callback = function(self)
        local items = {}
        for i, item in listfiles("DeltaHub\\Builds\\") do
            local fixedname = item:gsub("DeltaHub\\Builds\\","")
            fixedname = fixedname:gsub("DeltaHub/Builds","")
            fixedname = fixedname:gsub("\\","");fixedname=fixedname:gsub("/","")
            fixedname = fixedname:gsub("DeltaHubBuilds", "")
...
local l0 =info:Label({Text = "Rework By svotiff & Made by VateQ & CandyHub\ndiscord: vateq || discord.gg/EAbRQtEzWY\n"})
